...
// blue WAS the app's own brand accent (and is the ruler's original fixed
// color, kept first/default) at the time this was written; the amber
// reuses the app's one other established warm accent (#f7a062, the
// Dashboard "continue reading" highlight) rather than inventing a fresh
// orange.
//
// 2026-09-15: the app's brand accent moved to #002147 — this palette is
// DELIBERATELY left pointing at the old #004aad rather than following
// it. This is a closed, reader-facing enum (`RulerColor`), not a live
// theme token: a reader's ruler-colour CHOICE persists to localStorage
// as one of these exact literal strings (see STORAGE_KEY below), so
// silently repointing '#004aad' to a different hex would either orphan
// an already-persisted choice (no longer matches any option, if the
// literal changed) or silently repaint it to a colour that reader never
// picked (if only the swatch's hex changed under the same array slot,
// while old-saved sessions still say '#004aad'). This swatch is now a
// fixed, independent "blue" option in a fixed 6-colour set, no longer
// tied to whatever the live brand accent happens to be — same posture as
// the amber swatch already had relative to the Dashboard highlight.
