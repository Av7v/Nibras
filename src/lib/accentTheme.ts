 ...
 * this exact function on the navy accent that was current when this was
 * written (#004aad, 2026-08-12–2026-09-15) reproduces #004aad/#0060e0/
 * #004096/#003a87 byte-for-byte (lands on l=34, the same lightness
 * index.css's own comment already documents) — this is a faithful
 * generalization of the existing recipe, not a divergent new one.
 *
 * Unaffected by the 2026-09-15 move to #002147 as the new DEFAULT
 * accent: this function's own hue always comes from whichever page-tint
 * hex the reader picked (TINTS.blue/green/rose in index.css, App.tsx's
 * effect below), never from the current default --color-accent — so
 * nothing here needed to change alongside that swap. (It would NOT
 * reproduce #002147's own family if run on #002147 itself — its fixed
 * STARTING_LIGHTNESS=34 belongs to this function's own #004aad-era
 * self-check above, not to the new default; index.css derives the new
 * default family directly from #002147's own lightness instead, see
