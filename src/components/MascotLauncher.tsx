...
export function MascotLauncher({ variant, onActivate }: { variant: 'floating' | 'sidebar'; onActivate?: () => void }) {
    ...
    return (
      ...
      >
        {/* Lantern icon BESIDE the «مرشد نبراس» label on one line, aligned
            ...
            Size history: 24 -> 32px (Amal, via team-lead: "a bit
            ...
            menu, «مرتبة على الشريط حق القائمة») -> 34px (Amal, 2026-09-15,
            fresh ask: bigger again). Dropping the slot at 32px
            ...
            stopped reading as one aligned column. The fix that time was
            never "shrink the icon back down" — it was RESTORING the fixed
            18px-wide flex-none slot so the label column stays aligned
            while the icon itself overflows that slot symmetrically
            (rendered centred, overflowing evenly on both sides — 5px/side
            at 28px, 8px/side at this 34px) rather than being clamped to
            fit it. Going bigger again at 34px reuses that SAME slot
            mechanism unchanged, so the earlier "untidy" failure mode
            (which was specifically about the missing slot, not about
            size) has no reason to return; still clearly bigger than
            NavItem's 18px icons (task #370's "prominent helper" intent),
            just more emphatically so per Amal's fresh ask.
        ...
        <span className="flex w-[18px] flex-none items-center justify-center">
          ...
          <img src={lanternStaticUrl} width={34} height={34} alt="" aria-hidden="true" className="w-[34px] h-[34px] max-w-none flex-none" />
        </span>
  ...
  //
  // size=96 (was 64, Amal 2026-09-15: bigger). Lands exactly on
  // NibrasGuideFace's own documented "ideally 96" — its prop doc says the
  // talking mouth reads as a shimmer below 64px and is unambiguous at
  // 96px; this launcher renders idle (never talking) so that specific
  // guidance isn't strictly load-bearing here, but 96 is still the right
  // number to land on: it's vdesign's own measured ideal for this exact
  // SVG rather than an arbitrary pick inside Amal's ~80-96px ask, and the
  // button has no fixed width/height of its own (just `flex-none` +
  // `rounded-full`), so it grows to fit the face with no separate resize.
  ...
  return (
    ...
    >
      <NibrasGuideFace stateClass="expr-idle mouth-rest" size={96} />
    </button>
  )
}
